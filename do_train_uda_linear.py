...
def train(args, data_processor, uda_processor, model, tokenizer, role):
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for batch, uda_batch in zip(epoch_iterator, uda_dataloader):
            ...
            alpha = (t_total - global_step) / t_total
            uda_loss = alpha * consistency_loss_function(b_logits, a_logits)
            if args.n_gpu > 1:
            ...
            global_step += 1
            training_loss += (loss.item() + uda_loss.item())
            description = "Global step: {:>6d}, Loss: {:.4f}, UDA Loss: {:.4f} ({:.4f})".format(
                global_step, loss.item(), uda_loss.item(), alpha,
            )
...
def main():
    ...
    if args.local_rank in [-1, 0] and args.log_file is None and args.log_dir is not None:
        ...
        args.log_file = os.path.join(
            args.log_dir,
            "{}_{}_{}_{}_{}_linear_{:.1f}_{:.1e}.txt".format(
    ...
    if args.do_train:
        args.output_dir = os.path.join(
            args.output_dir,
            "{}_{}_{}_{}_{}_linear_{:.1f}_{:.1e}".format(
                list(filter(None, args.model_name_or_path.split("/"))).pop(),
                args.max_src_length,
                args.max_tgt_length,
                "raw" if not args.with_prefix else "prefix",
