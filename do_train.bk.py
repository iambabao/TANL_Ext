...
def evaluate(args, data_processor, model, split, prefix=""):
    ...
    eval_outputs = refine_outputs(args, eval_outputs, data_processor)
    ...
    eval_results = compute_metrics(args, eval_outputs, data_processor)
...
def main():
    ...
    # Parse tasks
    args.task_list = sorted(_.strip() for _ in args.tasks.strip().split(","))
    ...
    for task in args.task_list:
        for split in ["train", "valid", "test"]:
            ...
            logger.info("{}-{}: {}".format(task, split, len(datasets[task][split].input_sentences)))
